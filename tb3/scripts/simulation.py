...
if __name__ == '__main__':
    ...
    dt = 0.1
	#四个机器人的初始位置
    init_pos = [[2, 5, 0], [6, 5, 3.14], [1, -4, 1.57],[7, -4, 1.57]]
...
# 判断机器人是否订阅到位置
        ...
        # follow 0 pid controller
        if  dist(pos[0], pos[1]) > dist_thod_f:
            goal_angle_x = math.atan2(
                pos[0][1] - pos[1][1], pos[0][0] -
                pos[1][0]) - pos[1][2]
            speed_x = saturation(0.5 * dist(pos[0], pos[1]), 0, max_speed)
            angular_x = angle_adjust(goal_angle_x)
            follower[0].publishVelocityCommand(speed_x, angular_x)
        else:
            follower[0].stop()


        # follow 1 DWA controller
        if dist(pos[2], pos[1]) > dist_thod_f:
            speed_x, angular_x = dwa_controller.planning(pos[2],pos[1],obstacles = obstacles)
            follower[1].publishVelocityCommand(speed_x, angular_x)
        else:
            follower[1].stop()

        # follow 2 APF controller
        if dist(pos[3], pos[2]) > dist_thod_f:
            speed_x, angular_x = apf_controller.planning(pos[3], pos[2], obstacles=obstacles)
            follower[2].publishVelocityCommand(speed_x, angular_x)
        else:
            follower[2].stop()
